/*
 ...
 NOT DONE SOLVING YET-MAIN METHOD HAS TO BE SOLVED.
...
class Test{
    ...
    public int[] nextLargerNodes(Node head) {
        ...
        int[] oparr=new int[nvals.size()];
        Stack<Integer>stack=new Stack();
        for(int i=0;i<nvals.size();i++)
        {
            while(!stack.isEmpty() && nvals.get(stack.peek())<nvals.get(i)){
                oparr[stack.pop()]=nvals.get(i);
            }
            stack.push(i);
        }
        return oparr;
  ...
  /*  public void display()
    ...
    public static void main(String[] ars)
    {
		...
		System.out.println(list.nextLargerNodes(list.head));
